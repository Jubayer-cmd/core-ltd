...
export default function Counter() {
  ...
  useEffect(() => {
    ...
    let clockInterval = setInterval(() => {
      ...
      let dd = Math.max(0, Math.floor(remaining / day));
      let hh = Math.max(0, Math.floor((remaining % day) / hour));
      let mm = Math.max(0, Math.floor((remaining % hour) / min));
      let ss = Math.max(0, Math.floor((remaining % min) / sec));
